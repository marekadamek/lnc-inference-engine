...
object Parser {
        ...
        handleOperator(LImplToken, (p, q) => Impl(q, p),
          handleOperator(OrToken, (p, q) => Or(Set(p, q)),
            handleOperator(AndToken, (p, q) => And(Set(p, q)), {
              ...
              case (GloballyToken :: ts :: Nil, map) =>
                Globally(processBracket(ts, map))

              case (FinallyToken :: ts :: Nil, map) =>
                Finally(processBracket(ts, map))
  ...
  def parse(formula: String): Try[Expr] = Tokenizer.tokenize(formula) flatMap parse
}
