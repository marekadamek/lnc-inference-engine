...
object TestFormulas {
  ...
  def satisfiableC(n: Int, d: Int): (Expr, String) = {
    val base = Expr.or((for (i <- 1 to n) yield Var(s"p$i").asInstanceOf[Expr]).toSet)
    val f = C(d, base)
    (f, s"C_${d}_$n")
  }

  def unsatisfiableC(n: Int, d: Int): (Expr, String) = {
    val base = Expr.or((for (i <- 1 to n) yield Var(s"p$i").asInstanceOf[Expr]).toSet)
