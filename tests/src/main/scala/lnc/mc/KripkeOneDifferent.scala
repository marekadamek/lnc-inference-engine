...
object KripkeOneDifferent extends App with AppConfig {
  ...
  for (d <- 0 to 200 by 20) {
    ...
    ModelCheckingSMVExporter.convert(model, List(spec), Paths.get(targetDir, s"kripke_one_$d.smv"))
    ModelCheckingSPOTExporter.convert(model, List(spec), Paths.get(spotTargetDir, s"kripke_one_$d.spot"))
